...
Met behulp van een tool als. _exiftool_ kunnen embedded metadatatags zoals XMP en IPTC gelezen, geschreven of eventueel gekopieerd worden. Onderstaand commando geeft alle XMP en IPTC tags gegroepeerd per _tag family_ terug in JSON-formaat.

[source,shell]
$ exiftool 7659c97c0w.tif -XMP:All -IPTC:All -g0:1 -json

De resulterende json kan gebruikt worden om de relevante tags in de afgeleide jp2 file te schrijven. Het kan nodig zijn redundante en niet (langer) relevante tags eerst te verwijderen.
...
Single line json logging naar stdout op machine.
